...
Server mode means keeping HydraOps on 24/7 on a machine with no screen — a mini PC at home, an old laptop — and using it from the browser of any device on your network. It is also what makes [scheduled tasks](./08-scheduled-tasks.md) run at all times.
