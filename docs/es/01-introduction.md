...
- **Add-ons.** Add-ons nativos, add-ons tuyos y servidores MCP: lo que los agentes usan para *hacer*. Ver [Add-ons y MCP](./07-addons.md).
- **Herramientas.** Conectores hacia servicios externos, como Telegram para manejar los agentes desde el móvil. Ver [Herramientas (integraciones)](./08-tools.md).
- **Tareas programadas.** Crons: "cada mañana a las 8, resume las novedades de…". Ver [Tareas programadas](./09-scheduled-tasks.md).
...
- **Servidor (headless).** Un solo comando levanta la pila en una máquina sin pantalla —un mini PC en casa, por ejemplo— y la usas desde el navegador de cualquier equipo de tu red. Ver [Modo servidor](./11-server-mode.md).

## Dónde están tus datos

Tus agentes, mensajes, adjuntos y ajustes viven **fuera** del directorio de instalación (en Windows, `%APPDATA%\HydraOps`), así que actualizar o reinstalar la aplicación no toca nada tuyo. Las claves de API van aparte, en un almacén propio — ver [Seguridad](./12-security.md).
